...
mkdir -p \
  "$PI_HOME/extensions" \
  "$PI_HOME/extensions.disabled" \
  "$PI_HOME/prompts" \
  "$PI_HOME/themes" \
  "$AGENTS_HOME/skills" \
  "$AGENTS_HOME/skills.disabled" \
  "$AGENTS_HOME/shelf"

rsync -a --exclude='.gitkeep' "$ROOT/configs/pi-agent/" "$PI_HOME/"
rsync -a --exclude='.gitkeep' "$ROOT/extensions/" "$PI_HOME/extensions/"
rsync -a --exclude='.gitkeep' "$ROOT/extensions.disabled/" "$PI_HOME/extensions.disabled/"
...
rsync -a "$ROOT/skills.disabled/" "$AGENTS_HOME/skills.disabled/"
